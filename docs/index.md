...
K8s-only and Batch+K8s are nearly identical at **$28.11** and **$27.82** respectively — within 1% of each other. Both are significantly cheaper than plain Batch ($32.60) or Pulsar ($41.39). The trade-off: K8s-only avoids per-job VM provisioning but the n2-standard-16 sits partially idle and jobs cannot run concurrently.

## Key Findings

1. **K8s-only and Batch+K8s are the cheapest deployments** at $28.11 and $27.82 respectively. With GKE Standard management fees, the two approaches are essentially equivalent in cost despite very different architectures.
