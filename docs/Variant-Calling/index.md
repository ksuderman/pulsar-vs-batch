---
title: Pulsar vs Direct GCP Batch - Variant Calling
---

# Pulsar vs Direct GCP Batch: Variant Calling

**[Interactive Charts](charts.html)** | **[Cost Summary](costs.html)** | **[Cost Charts](cost-charts.html)**

## Experiment Setup

- **Workflow:** Variant Calling (12 unique tool types)
- **Galaxy version:** 26.1
...
|------|------------|-------------------|---------------------|
| lofreq_call | 2 | 0.97x | ~2x |
| bwa_mem | 8 | 0.96x | ~8x |
| picard_MarkDuplicates | 3 | 0.53x | ~3x |
| lofreq_viterbi | 4 | 0.79x | ~4x |
| snpEff_build_gb | 1 | 0.16x | ~1x |
| lofreq_indelqual | 1 | 0.63x | ~1x |
| fastp | 4 | 0.96x | ~4x |
| samtools_view | 1 | 0.74x | ~1x |
| samtools_stats | 2 | 0.90x | ~2x |
| multiqc | 1 | 0.33x | ~1x |
...
### 1. 2GB: Pulsar is +14% slower (wall clock)

- Wall clock: 230.6m vs 201.6m
- Compute: 155.3m vs 182.9m (-15%)

### 2. 5GB: Pulsar is +10% slower (wall clock)

- Wall clock: 372.4m vs 339.4m
- Compute: 276.0m vs 320.1m (-14%)

### 3. 10GB: Pulsar is +11% slower (wall clock)
...
- **picard_MarkDuplicates** (2GB): -47% (292s vs 547s)
- **lofreq_viterbi** (2GB): -21% (287s vs 363s)
- **snpEff_build_gb** (2GB): -84% (176s vs 1097s)
- **lofreq_indelqual** (2GB): -37% (158s vs 250s)
- **samtools_view** (2GB): -26% (101s vs 136s)
- **multiqc** (2GB): -67% (3s vs 9s)
